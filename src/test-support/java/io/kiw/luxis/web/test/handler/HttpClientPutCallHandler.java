...
public class HttpClientPutCallHandler extends JsonHandler<HttpClientPostRequest, HttpClientGetResponse, MyApplicationState> {

    private final LuxisHttpClient httpClient;

    public HttpClientPutCallHandler(final LuxisHttpClient httpClient) {
    ...
    public RequestPipeline<HttpClientGetResponse> handle(final HttpStream<HttpClientPostRequest, MyApplicationState> httpStream) {
        return httpStream
            .<HttpClientResponse>asyncMap(ctx -> httpClient.put("http://serverB" + ctx.in().targetPath, ctx.in().forwardBody))
