...
public class SplitWebSocketRoutes extends WebSocketRoutes<MyApplicationState, TestWebSocketResponse> {
    ...
    private static IndividualMessageWebSocketPipeline<WebSocketEchoResponse> handleEcho(WebSocketStream<WebSocketEchoRequest, MyApplicationState, TestWebSocketResponse> stream) {
        return stream
                .map(ctx -> new WebSocketEchoResponse("echo: " + ctx.in().message))
                .complete();
    }

    private static IndividualMessageWebSocketPipeline<WebSocketNumberResponse> handleNumber(WebSocketStream<WebSocketNumberRequest, MyApplicationState, TestWebSocketResponse> stream) {
