...
public class SplitWebSocketRoutes extends WebSocketRoutes<MyApplicationState, TestWebSocketResponse> {
    ...
    private static WebSocketPipeline<WebSocketEchoResponse> handleEcho(WebSocketStream<WebSocketEchoRequest, MyApplicationState, TestWebSocketResponse> stream) {
        return stream
                .map(ctx -> new WebSocketEchoResponse("echo: " + ctx.in().message))
                .complete();
    }

    private static WebSocketPipeline<WebSocketNumberResponse> handleNumber(WebSocketStream<WebSocketNumberRequest, MyApplicationState, TestWebSocketResponse> stream) {
