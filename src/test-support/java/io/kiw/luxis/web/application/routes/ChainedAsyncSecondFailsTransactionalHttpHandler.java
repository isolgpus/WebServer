...
public class ChainedAsyncSecondFailsTransactionalHttpHandler implements JsonHandler<EchoRequest, EchoResponse, MyApplicationState> {
    ...
    public LuxisPipeline<EchoResponse> handle(final HttpStream<EchoRequest, MyApplicationState> e) {
        return e.map(ctx -> ctx.in().stringExample)
                .inTransaction(tx -> tx
                        .asyncMap(ctx -> AsyncTestSupport.<String, HttpErrorResponse>completed(ctx.in() + "-first"))
                        .<String>asyncMap(ctx -> AsyncTestSupport.failed(new RuntimeException("second async driver failed")))
