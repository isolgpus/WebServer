...
public class AsyncMapSyncThrowTransactionalHttpHandler implements JsonHandler<EchoRequest, EchoResponse, MyApplicationState> {
    ...
    public LuxisPipeline<EchoResponse> handle(final HttpStream<EchoRequest, MyApplicationState> e) {
        return e.map(ctx -> ctx.in().stringExample)
                .inTransaction(tx -> tx
                        .<String>asyncMap(ctx -> {
                            throw new RuntimeException("async mapper threw");
                        })
