...
The pipeline API forces you to declare whether each step is blocking, non-blocking, or async. Put blocking work on the event loop? **It won't compile.** The category of bug that causes 2am pages — thread starvation, blocked event loops, race conditions — becomes a red squiggle in your IDE.
