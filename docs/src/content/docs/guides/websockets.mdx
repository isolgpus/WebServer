...
- **`map` / `flatMap`** run on the event loop with access to application state
- **`blockingMap` / `blockingFlatMap`** run on a worker thread, no application state access (compiler-enforced)
