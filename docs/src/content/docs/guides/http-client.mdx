...
public static AppState registerRoutes(RoutesRegister routesRegister) {
    ...
    routesRegister.jsonRoute("/orders", Method.POST, state,
...
```java
public class CreateOrderHandler extends JsonHandler<OrderRequest, OrderResponse, AppState> {
...
Luxis<AppState> orderService = Luxis.test(r -> {
    AppState state = new AppState();
    r.jsonRoute("/orders", Method.POST, state, new CreateOrderHandler(httpClient));
    return state;
});
```
